...
export default function RegistrationStep1({ formData, onNextStep }) {
    const [restaurantName, setRestaurantName] = useState( formData.restaurantName || '');
    const [address, setAddress] = useState( formData.address || '');
    const [phoneNumber, setPhoneNumber] = useState( formData.phoneNumber || '');
    const [alternateNumber, setAlternateNumber] = useState(formData.alternateNumber || '');
    ...
    const [ownerEmail, setOwnerEmail] = useState(formData.ownerEmail || '');

    const handleNext = () => {
        // Implement validation logic here
        if(restaurantName && address && phoneNumber.length === 10 && email && password && confirmPassword && ownerName && ownerPhone && ownerEmail){
            onNextStep({
                restaurantName,
                address,
                phoneNumber,
